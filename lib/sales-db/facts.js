...
const { storeId, exactKeys, decimal, saleTime, identifier } = require('./values');
...
function createReviewedCatalog({ products, payments }) {
  ...
  const approvedPayments = new Set(payments.map(entry => {
    ...
    return JSON.stringify([label(entry.paymentType), entry.paymentCode === null ? null : identifier(entry.paymentCode)]);
  }));
  return Object.freeze({ validate(line) {
