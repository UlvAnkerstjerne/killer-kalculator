...
const { processLines, toCphDate } = require('./sales-lines');
...
async function fetchSalesRange({ store, start, end, httpGet }) {
  ...
  // ── Early-exit state ───────────────────────────────────────────────────────
  //
  // OnlinePOS returns pages in approximately ascending timestamp_pay order, but
  // NOT with strict monotonicity: retroactive entries (corrections, late-posted
  // refunds) can have old timestamp_pays and appear interspersed with newer lines,
  // including across page boundaries.
  //
  // Safe early-exit condition (verified against live data 2026-09-22):
  //   Once a page's MINIMUM valid CPH date is >= end, all lines on that page and
  //   all future pages are out of range.  Using min (not last element or max)
  //   ensures retroactive entries that appear at the END of a page are not
  //   overlooked — those entries have old timestamps and would lower the page min.
  //
  // Fail-closed condition (non-monotonic data):
  //   If a page's minimum valid CPH date is earlier than the running global minimum
  //   seen across all previous pages, it indicates data arrived significantly out of
  //   order.  In that case earlyExitSafe is set to false for the remainder of the
  //   fetch, preventing silent loss of in-range lines that might still be arriving
  //   on future pages.  The fetch continues to MAX_PAGES as a backstop.
  //
  // Empty pages and pages with no valid timestamps are passed through without
  // triggering early exit.
  let earlyExitSafe  = true;   // set to false on detected min-date regression
  let globalMinDate  = null;   // running minimum of page min-dates seen so far
  ...
  while (nextUrl !== null) {
    ...
    // ── Early-exit check ────────────────────────────────────────────────────
    if (rawNext !== null && pageData.length > 0) {
      // Compute the minimum valid CPH date across this page.
      // Lines with null/invalid timestamp_pay are skipped; they do not affect
      // the min-date and do not prevent early exit.
      let pageMinDate = null;
      for (const line of pageData) {
        const d = toCphDate(line.timestamp_pay ?? line.datetime ?? null);
        if (d !== null && (pageMinDate === null || d < pageMinDate)) {
          pageMinDate = d;
        }
      }

      if (pageMinDate !== null) {
        // Fail-closed: if this page's min date is earlier than the global minimum
        // seen so far, the data is arriving out of order.  Disable early exit to
        // avoid silently losing in-range lines that might still appear later.
        if (globalMinDate !== null && pageMinDate < globalMinDate) {
          earlyExitSafe = false;
        }
        // Update the running global minimum.
        if (globalMinDate === null || pageMinDate < globalMinDate) {
          globalMinDate = pageMinDate;
        }

        // Early exit: if this page's minimum CPH date is at or past the exclusive
        // end date, this page has no in-range lines and neither will any future page.
        if (earlyExitSafe && pageMinDate >= end) {
          break;   // rawNext was non-null but we stop requesting further pages
        }
      }
    }
