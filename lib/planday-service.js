...
async function loadPayrollSources({ client, window, now = Date.now }) {
  ...
  if (employees.length) {
    ...
    allocationShifts = await client.all('/scheduling/v1/shifts', { from: monthStart, to: nextDate(monthEnd, -1) });
  }
  ...
  return { payroll, approved, shifts, departments, allocations, memberships, allocationShifts, allocationCoverage };
}
...
function createPayrollService({ client, now = Date.now, maxEntries = 64, maxPending = 16 }) {
  ...
  async function fetchPayroll(window) {
    try {
      return processPayroll(normalizePayroll(await loadPayrollSources({ client, window, now }), window), window);
    } catch (err) {
