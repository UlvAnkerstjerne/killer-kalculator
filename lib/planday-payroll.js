...
// Verified independent operations; these are allocation destinations, not stores.
const EXCLUDED_DEPARTMENTS = Object.freeze({ 149683: 'Parken B', 149749: 'Parken C', 149787: 'Festival', 149750: 'Office' });
function excludedDepartment(id) { return Object.hasOwn(EXCLUDED_DEPARTMENTS, id); }
...
const CODES = new Set(['UPSTREAM_UNAVAILABLE', 'INVALID_SCHEMA', 'PAGINATION_INCOMPLETE',
  ...
  'ALLOCATION_HISTORY_UNAVAILABLE', 'SALARY_SCOPE_UNDECIDED', 'SALARY_HOURS_UNAVAILABLE', 'MINUTE_OVERLAP_ESTIMATE', 'SOURCE_COVERAGE_INCOMPLETE', 'ADJUSTMENT_ALLOCATION_ESTIMATE',
  'ACTUAL_HOURS_MISSING', 'ATTENDANCE_CONFLICT', 'ATTENDANCE_UNAPPROVED', 'ATTENDANCE_BREAKS_UNAVAILABLE',
  'OVERLAPPING_WORK', 'SHIFT_TYPE_UNAVAILABLE', 'WORKER_RULE_CONFIGURATION', 'WORKER_RULE_UNAVAILABLE',
  'SALARY_MONTH_UNAVAILABLE', 'SICK_LEAVE_WITHOUT_MONETARY_PAY', 'SCHEDULED_HOURS_FALLBACK', 'CALENDAR_SALARY_FALLBACK']);
...
function processPayroll({ records = [], failures = [], warnings = [], storeWarnings = [], covered = true, coverage, hours = [], estimatedDepartments = [] }, window) {
  const bags = Object.fromEntries(STORE_IDS.map(id => [id, { amounts: Object.fromEntries(COMPONENTS.map(x => [x, 0])), sources: [], warnings: new Set(), reasons: new Set(), calendarDays: 0 }]));
  const globalWarnings = new Set(warnings.map(safeCode));
  for (const warning of storeWarnings) {
    const id = storeForDepartment(warning.departmentId);
    if (id) {
      bags[id].warnings.add(safeCode(warning.code));
      if (warning.code === 'CALENDAR_SALARY_FALLBACK') bags[id].sources.push('estimated');
    }
  }
  ...
  for (const r of records) {
    try {
      ...
      const fingerprint = JSON.stringify([r.departmentId, r.start, r.end, r.amount, r.source, r.component, r.hourly, r.prorate, r.calendarFallbackDays]);
      if (seen.has(r.id)) {
      ...
      if (r.calendarFallbackDays === 1 && r.component === 'salaried' && source === 'estimated') bag.calendarDays++;
    } catch (err) { fail(err.message, r.departmentId); }
  }
  const stores = {};
  const provenance = Object.fromEntries(STORE_IDS.map(id => [id, { actual: 0, fallback: 0, keys: new Set(), estimated: false }]));
  for (const id of estimatedDepartments) if (storeForDepartment(id)) provenance[storeForDepartment(id)].estimated = true;
  for (const h of hours) {
    const id = storeForDepartment(h.departmentId);
    if (!id) continue;
    const duration = Math.max(0, Math.min(h.end, window.until) - Math.max(h.start, window.from));
    if (!Number.isFinite(duration) || !duration) continue;
    if (h.source === 'actual') provenance[id].actual += duration;
    else {
      provenance[id].fallback += duration; provenance[id].estimated = true;
      for (const key of h.fallbackKeys || []) provenance[id].keys.add(key);
    }
  }
  const roundedHours = ms => Math.round(ms / 3600000 * 1000000) / 1000000;
  for (const id of STORE_IDS) {
    const bag = bags[id], complete = bag.reasons.size === 0;
    const hp = provenance[id];
    if (hp.estimated) { bag.sources.push('estimated'); bag.warnings.add('SCHEDULED_HOURS_FALLBACK'); }
    ...
    stores[id] = { cost: complete ? Object.values(componentOre).reduce((s, n) => s + n, 0) / 100 : null,
      ...
      actualHours: roundedHours(hp.actual), scheduledFallbackHours: roundedHours(hp.fallback),
      scheduledFallbackShifts: hp.keys.size, estimated: sourceOf(bag.sources) !== 'actual',
      calendarFallbackDays: bag.calendarDays,
  ...
  return { stores, chain: { cost: complete ? STORE_IDS.reduce((s, id) => s + Math.round(stores[id].cost * 100), 0) / 100 : null,
    ...
    actualHours: roundedHours(Object.values(provenance).reduce((n, h) => n + h.actual, 0)),
    scheduledFallbackHours: roundedHours(Object.values(provenance).reduce((n, h) => n + h.fallback, 0)),
    scheduledFallbackShifts: new Set(Object.values(provenance).flatMap(h => [...h.keys])).size,
    calendarFallbackDays: STORE_IDS.reduce((n, id) => n + stores[id].calendarFallbackDays, 0),
    estimated: STORE_IDS.some(id => stores[id].estimated) },
    ...
    ...(coverage ? { coverage: Object.fromEntries(['attendanceRequired', 'attendanceMatched', 'attendanceMissing',
      'attendanceUnapproved', 'attendanceInvalid', 'sickLeaveExcluded'].map(k => [k,
      Number.isSafeInteger(coverage[k]) && coverage[k] >= 0 ? coverage[k] : 0])) } : {}) };
}
module.exports = { DEPARTMENTS, EXCLUDED_DEPARTMENTS, excludedDepartment, STORE_IDS, dateValid, nextDate, cphDate, midnight, localInstant, period, splitOre,
