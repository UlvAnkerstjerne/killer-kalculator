...
async function loadAttendance({ client, input, window, workerRules, now }) {
  ...
  if (candidates.size) {
    try {
      ...
      attendance = pages.flat().filter(a => candidates.has(a.employeeId));
  ...
  for (const s of [...schedule, ...input.shifts]) {
    ...
    const missing = storeForDepartment(s.departmentId) && !input.payroll.shiftsPayroll.some(p => p.id === s.id) &&
  ...
  await batches([...needed.values()], async s => {
    const { data } = await client.get('/scheduling/v1/shifts/' + encodeURIComponent(s.id));
    if (!data || data.id !== s.id || data.employeeId !== s.employeeId || data.departmentId !== s.departmentId ||
        data.startDateTime !== s.startDateTime || data.endDateTime !== s.endDateTime || data.status !== s.status) throw new Error();
    // Live normal shifts omit nullable shiftTypeId; typed absence responses
    // carry it. Absence classification never uses a fallback monetary rate.
    details.set(s.id, { shiftTypeId: data.shiftTypeId ?? null });
  });
  ...
  return { ...input, workerRules, evaluatedAt: now(), monthly, attendance, attendanceBreaks: breaks,
    shiftDetails: details, sickTypes, timeCosts };
}
