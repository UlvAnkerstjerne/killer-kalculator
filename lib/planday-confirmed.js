...
// Calendar dates carry equal salary weights, including weekends and DST days.
...
function calendarSalary(p, month, departmentId, window, index) {
  ...
  const totalOre = Math.round(p.salary * 100), dailyOre = Math.trunc(totalOre / dates.length);
  if (!included.length) return [];
  const amountOre = Math.round(totalOre * (included.at(-1).i + 1) / dates.length) - Math.round(totalOre * included[0].i / dates.length);
  return included.map((day, i) => ({ id: 'salary-calendar:' + index + ':' + day.i, departmentId,
    start: Math.max(day.start, window.from), end: Math.min(day.end, window.until),
    amount: (i === included.length - 1 ? amountOre - dailyOre * i : dailyOre) / 100,
