...
// Calendar dates carry equal salary weights; an unfinished date accrues by
// elapsed time between its Copenhagen midnights, including 23/25-hour DST days.
// Round cumulative month shares so adjacent requests conserve the same øre;
// put the requested slice's rounding residual on its final included date.
function calendarSalary(p, month, departmentId, window, index) {
  const dates = [];
  for (let date = month.from; date <= month.to; date = nextDate(date)) dates.push(date);
  const included = dates.map((date, i) => ({ i, start: midnight(date), end: midnight(nextDate(date)) }))
    .filter(day => day.start < window.until && day.end > window.from);
  const totalOre = Math.round(p.salary * 100);
  if (!included.length) return [];
  const progress = (day, instant) => day.i + Math.max(0, Math.min(1, (instant - day.start) / (day.end - day.start)));
  const amountOre = Math.round(totalOre * progress(included.at(-1), window.until) / dates.length) -
    Math.round(totalOre * progress(included[0], window.from) / dates.length);
  let allocatedOre = 0;
  return included.map((day, i) => {
    const start = Math.max(day.start, window.from), end = Math.min(day.end, window.until);
    const shareOre = i === included.length - 1 ? amountOre - allocatedOre :
      Math.trunc(totalOre / dates.length * (end - start) / (day.end - day.start));
    allocatedOre += shareOre;
    return { id: 'salary-calendar:' + index + ':' + day.i, departmentId, start, end, amount: shareOre / 100,
      source: 'estimated', component: 'salaried', calendarFallbackDays: 1 };
  });
}
...
function normalizeConfirmed(input, window, standard) {
  ...
  const ruleFor = s => {
    ...
    if (rule?.kind === 'home' && !dateValid(date)) return salaryRows.some(({ p }) => p.employeeId === s.employeeId && p.start >= rule.from) ? rule : undefined;
  ...
  const uniqueSchedules = new Map(), unreliableHomeSchedules = [];
  for (const s of [...input.allocationShifts, ...input.shifts]) {
    const previous = uniqueSchedules.get(s.id);
    if (previous && ['employeeId', 'departmentId', 'date', 'timeZone', 'startDateTime', 'endDateTime', 'status', 'shiftTypeId'].some(k => (previous[k] ?? null) !== (s[k] ?? null))) {
      if (previous.employeeId === s.employeeId && ruleFor(s)?.kind === 'home') unreliableHomeSchedules.push(previous, s);
      else fail('CONFLICTING_DUPLICATE', s.departmentId);
    }
  ...
  for (const [index, { p, month }] of [...uniqueSalaries.values()].entries()) {
    ...
    const own = allSchedules.filter(s => s.employeeId === p.employeeId &&
      (rule.kind === 'home' && !dateValid(s.date) || s.date >= month.from && s.date <= month.to || punchedOverlap(s, monthFrom, monthUntil) ||
    ...
    try {
      if (!Number.isFinite(p.salary) || !Number.isSafeInteger(Math.round(p.salary * 100)) || !dateValid(p.start) || !dateValid(p.end) || p.start > p.end || !['central', 'home', 'regional'].includes(rule.kind) || p.start < month.from || p.end > month.to || p.start < rule.from) throw new Error('SALARY_MONTH_UNAVAILABLE');
      if (rule.kind === 'home' && (!storeForDepartment(rule.departmentId) || !knownDepartments.has(rule.departmentId))) throw new Error('UNKNOWN_DEPARTMENT');
      const completeSchedule = input.allocationCoverage && input.allocationCoverage.from <= month.from && input.allocationCoverage.end >= nextDate(month.to);
      if (!completeSchedule && rule.kind !== 'home') throw new Error('SALARY_HOURS_UNAVAILABLE');
      ...
      const work = []; let invalid = !completeSchedule || unreliableHomeSchedules.some(s => s.employeeId === p.employeeId &&
        (!dateValid(s.date) || s.date >= month.from && s.date <= month.to || own.some(o => o.id === s.id)));
      // A known Payroll shift absent from the monthly schedule is positive
      // evidence of incomplete coverage; a merely quiet month is not.
      if (rule.kind === 'home' && month.payroll.shiftsPayroll.some(s => s.employeeId === p.employeeId && !own.some(o => o.id === s.id))) invalid = true;
      for (const s of own) {
        let start, end;
        try { start = localInstant(s.startDateTime); end = localInstant(s.endDateTime); } catch {
          if (!dateValid(s.date)) { invalid = true; if (rule.kind !== 'home') failSalary('ACTUAL_HOURS_MISSING'); continue; }
          start = midnight(s.date); end = midnight(nextDate(s.date));
        }
        ...
        try {
          ...
          else if (rule.kind !== 'home') failSalary(err.message);
        }
      }
      ...
      for (const a of input.attendance || []) if (a.employeeId === p.employeeId && !own.some(s => s.id === a.shiftId)) {
        try {
          if (localInstant(a.startDateTime) >= agreementUntil || (a.endDateTime && localInstant(a.endDateTime) <= agreementFrom)) continue;
          if (rule.kind === 'central' && (!storeForDepartment(a.departmentId) || localInstant(a.startDateTime) >= window.until ||
              (a.endDateTime && localInstant(a.endDateTime) <= window.from))) continue;
        } catch { /* Unusable unmatched attendance cannot establish complete monthly weights. */ }
        invalid = true; if (rule.kind !== 'home') failSalary('ATTENDANCE_CONFLICT');
      }
      ...
      if (rule.kind === 'home' && (invalid || !work.some(s => s.end > Math.max(s.start, agreementFrom) && s.start < agreementUntil))) {
        const totalOre = Math.round(p.salary * 100);
        extra.push(...calendarSalary(p, month, rule.departmentId, window, index));
        storeWarnings.push({ code: 'CALENDAR_SALARY_FALLBACK', departmentId: rule.departmentId });
        reconciliation.expectedSalaryOre += totalOre; reconciliation.allocatedSalaryOre += totalOre;
