...
function normalizePayroll(input, window) {
  if (input.workerRules) return require('./planday-confirmed').normalizeConfirmed(input, window, normalizeStandardPayroll);
  return normalizeStandardPayroll(input, window);
}
function normalizeStandardPayroll({ payroll, approved, shifts, departments, allocations, memberships = new Map(), allocationShifts = shifts, allocationCoverage,
  skipSalaryAllocation = false, ignoreShift = () => false, absenceExclusions = new Set(), monetaryAbsences = new Set(), outsideDepartment = excludedDepartment }, window) {
  const excludedDepartment = outsideDepartment;
  ...
  const schedule = unique(shifts.filter(s => (!ignoreShift(s) || adjustedEmployees.has(s.employeeId)) &&
    (!excludedDepartment(s.departmentId) || salariedEmployees.has(s.employeeId) || adjustedEmployees.has(s.employeeId))), shiftKey);
  ...
  for (const p of pays.values()) {
    try {
      ...
      if (p.salary === 0 && !Number.isFinite(p.wage?.rate) && !monetaryAbsences.has(p.id)) throw new Error('MISSING_PAY_DATA');
      records.push(row);
    } catch (err) { fail(err.message, p.departmentId); }
  }
  for (const s of schedule.values()) {
    if (OMIT.has(s.status)) continue;
    // Managed work is validated by the punch/schedule hierarchy. Its ordinary
    // coverage pass must not reject good punches for missing scheduled times.
    if (ignoreShift(s) && !adjustedEmployees.has(s.employeeId)) continue;
    try {
      ...
      if (!ignoreShift(s) && !absenceExclusions.has(s.id) && !pays.has(s.id) && !payroll.salariedPayroll.some(p => p.employeeId === s.employeeId && p.start <= s.date && s.date <= p.end)) fail('SOURCE_COVERAGE_INCOMPLETE', s.departmentId);
  ...
  for (const s of unique(allocationShifts.filter(s => salariedEmployees.has(s.employeeId) && !ignoreShift(s)), shiftKey).values()) {
  ...
  for (const [i, p] of [...salaryRecords.values()].filter(p => !skipSalaryAllocation ||
    (typeof skipSalaryAllocation === 'function' && !skipSalaryAllocation(p))).entries()) {
    try {
