...
function createImportRepository(session, context) {
  ...
  async function verify(run) {
    if (!run.verificationOf) fail('INVALID_RUN');
    await session.transaction(async () => {
      await identity();
      const { rows: [scan] } = await session.query(`SELECT status, terminal, verified, verification_of
        FROM sales_foundation.sales_import_scan WHERE run_id = $1 AND store_id = $2 FOR UPDATE`,
      [run.id, run.store]);
      if (scan?.status === 'published' && scan.terminal && scan.verified && scan.verification_of === run.verificationOf) return;
      if (scan?.status !== 'validated' || !scan.terminal || !scan.verified || scan.verification_of !== run.verificationOf) fail('INVALID_RUN');

      // Verification is deliberately metadata-only. Compare every persisted
      // allowlisted fact field, but never INSERT/UPDATE/DELETE sales_line.
      const factFields = FIELDS.map(([sql]) => sql).join(', ');
      const mismatch = await session.query(`SELECT 1 FROM
        (SELECT ${factFields} FROM sales_foundation.sales_stage_line
          WHERE run_id = $1 AND store_id = $2 AND business_date >= $3 AND business_date < $4) s
        FULL JOIN
        (SELECT ${factFields} FROM sales_foundation.sales_line
          WHERE store_id = $2 AND business_date >= $3 AND business_date < $4) p
        USING (store_id, source_key)
        WHERE s.source_key IS NULL OR p.source_key IS NULL OR
          ROW(${FIELDS.map(([sql]) => `s.${sql}`).join(', ')}) IS DISTINCT FROM
          ROW(${FIELDS.map(([sql]) => `p.${sql}`).join(', ')}) LIMIT 1`,
      [run.id, run.store, run.start, run.end]);
      if (mismatch.rows.length) fail('VERIFICATION_MISMATCH');

      const updated = await session.query(`UPDATE sales_foundation.sales_day_state d SET
          verified_at = clock_timestamp(),
          evidence = CASE WHEN i.line_count = 0 THEN 'verified-empty' ELSE 'independently-verified' END,
          verification_run = $1
        FROM sales_foundation.sales_import_day i
        WHERE i.run_id = $1 AND d.store_id = i.store_id AND d.business_date = i.business_date
          AND d.store_id = $2 AND d.business_date >= $3 AND d.business_date < $4
          AND d.line_count = i.line_count AND d.revenue_incl = i.revenue_incl
          AND d.revenue_excl = i.revenue_excl AND d.content_digest = i.content_digest`,
      [run.id, run.store, run.start, run.end]);
      const expected = (await session.query(`SELECT count(*)::int AS count FROM sales_foundation.sales_import_day
        WHERE run_id = $1 AND store_id = $2 AND business_date >= $3 AND business_date < $4`,
      [run.id, run.store, run.start, run.end])).rows[0].count;
      if (updated.rowCount !== expected) fail('VERIFICATION_MISMATCH');
      await session.query(`UPDATE sales_foundation.sales_sync_run SET state = 'published', published_at = clock_timestamp()
        WHERE run_id = $1 AND state = 'staging'`, [run.id]);
      await session.query(`UPDATE sales_foundation.sales_import_scan SET status = 'published', error_code = NULL
        WHERE run_id = $1 AND status = 'validated' AND verified`, [run.id]);
    });
    await purge(run.id);
  }
  async function failed(run, code) {
    ...
    // Invalid source facts (including missing/malformed identities) need review,
    // just like conflicting content; they are not operational failures.
    const status = ['INVALID_LINE', 'CATALOG_REVIEW', 'SOURCE_CONFLICT', 'RECONCILIATION_REQUIRED', 'VERIFICATION_MISMATCH'].includes(code)
  ...
  return Object.freeze({ begin, stage, progress, finishScan, preflight, publish, verify, failed, resume, coverage, ordered, report });
}
