...
async function traverse({ start, storeSlug, companyId, context, request, sink, signal,
  ...
  let next = initial, pages = 0, rows = 0, reviewCount = 0;
  ...
  while (next !== null) {
    ...
    await sink.progress({ pages, rows, reviewCount });
  }
