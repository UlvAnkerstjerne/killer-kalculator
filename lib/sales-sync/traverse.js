...
{ inspectAllDates = false, catalogReviewRow } = {}) {
  if (catalogReviewRow !== undefined && (typeof catalogReviewRow !== 'function' || inspectAllDates)) fail('INVALID_OPTIONS');
  ...
  while (next !== null) {
    ...
    for (let position = 0; position < envelope.data.length; position++) {
      ...
      // Explicit review-only projection: no normalizer, identities or facts.
      // Kept outside caller-supplied limits and unreachable from import modes.
      if (catalogReviewRow) { catalogReviewRow(raw); continue; }
      let line;
